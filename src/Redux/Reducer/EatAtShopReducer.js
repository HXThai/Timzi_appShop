import { ADD_TOPPING, UPDATE_QUANTITY, UPDATE_TEMP_COUNT, UPDATE_SIZE, UPDATE_CART, EAT_AT_SHOP } from '../Action/EatAtShop';
...
export default function async(state = initialState, action) {
    switch (action.type) {
        ...
        case UPDATE_CART: {
            if (action.payload.category !== CATEGORY.EAT_AT_SHOP) return {
                ...state
            }
            if (action.payload.calculation == CALCULATION.SUBTRACTION) {
                state.data.category_food[action.payload.value.itemFood.index].food[action.payload.value.itemFood.indexitemfood].book_food[
                ...
                state.data.category_food[action.payload.value.itemFood.index].food[action.payload.value.itemFood.indexitemfood].book_food[
                    ...
                    state.data.category_food[action.payload.value.itemFood.index].food[action.payload.value.itemFood.indexitemfood].book_food.splice(
                        action.payload.value.itemCard.index,
                        1
                    );
            } else {
        ...
        case ADD_TOPPING: {
            ...
            const { indexCategory, indexFood, indexCategoryTopping, indexToppingFood, type } = action.payload.data;
            var data =
                type == TYPE_UPDATE_CART.CATEGORY
                    ? state.data.category_food[indexCategory].food[indexFood]
                    : type == TYPE_UPDATE_CART.PROGRAM ? state.data.program_store[indexCategory].food[indexFood] :
            ...
            position == -1 && data2.id != -1 ? data.temp_arr_topping.push(data1.id) : data2.id != -1 && data.temp_arr_topping.splice(position, 1);
        ...
        case UPDATE_SIZE: {
            ...
            const { indexToppingFood, indexCategory, indexFood, indexCategoryTopping, type } = action.payload.data;
            var data =
                type == TYPE_UPDATE_CART.CATEGORY
                    ? state.data.category_food[indexCategory].food[indexFood]
                    : type == TYPE_UPDATE_CART.PROGRAM ? state.data.program_store[indexCategory].food[indexFood] :
            ...
            action.payload.data.itemTopping.topping_food.map((item, index) => {
                if (indexToppingFood == index) {
                    data.category_topping_food[indexCategoryTopping].topping_food[index].check = 1;
                    data.money_topping = (data.money_topping + (item.price) - money);
                    data.temp_size = data.category_topping_food[indexCategoryTopping].topping_food[index].id;
                } else {
                    data.category_topping_food[indexCategoryTopping].topping_food[index].check = 0;
                }
            });
            data.total_temp = data.money_food + data.money_topping * data.count_temp
            return {
        ...
        case UPDATE_TEMP_COUNT: {
            ...
            var data =
                action.payload.type == TYPE_UPDATE_CART.CATEGORY
                    ? state.data.category_food[action.payload.value.index].food[action.payload.value.indexitemfood]
                    : action.payload.type == TYPE_UPDATE_CART.PROGRAM ? state.data.program_store[action.payload.value.index].food[action.payload.value.indexitemfood] :
                        state.data.category_store_food[action.payload.value.index].food[action.payload.value.indexitemfood];
        ...
        case UPDATE_QUANTITY: {
            ...
            var tempArr =
                action.payload.type === TYPE_UPDATE_CART.CATEGORY
                    ...
                    : action.payload.type === TYPE_UPDATE_CART.PROGRAM
                        ...
                        : action.payload.type === TYPE_UPDATE_CART.COMBO ? state.data.combo_food : state.data.category_store_food;

            if (action.payload.type === TYPE_UPDATE_CART.PROGRAM) {
                reactotron.log("reducesr", action.payload)
                action.payload.status == CALCULATION.ADD && tempArr[action.payload.index].food[action.payload.indexitemfood].count_book_food++;
                action.payload.status == CALCULATION.SUBTRACTION && tempArr[action.payload.index].food[action.payload.indexitemfood].count_book_food > 0 && tempArr[action.payload.index].food[action.payload.indexitemfood].count_book_food--;
            ...
            if (action.payload.type === TYPE_UPDATE_CART.COMBO) {
                action.payload.status == CALCULATION.ADD && tempArr[action.payload.data.index].count_book_food++
                action.payload.status == CALCULATION.SUBTRACTION && tempArr[action.payload.data.index].count_book_food > 0 && tempArr[action.payload.data.index].count_book_food--
...
const spShopDetailReducer = async (data, check, callback) => {
    ...
    for (let index = 0; index < program_store.length; index++) {
        if (program_store[index].food.length > 0) {
            for (let index1 = 0; index1 < program_store[index].food.length; index1++) {
                Object.assign(program_store[index].food[index1], { count_temp: 0, temp_arr_topping: [], total_temp: 0, temp_size: null, money_topping: 0, money_food: 0 });
