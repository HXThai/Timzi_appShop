                ...
                onPressAdd={({ itemfood, index, indexitemfood }) => {
                  reactotron.log("réafsda", itemfood, index, indexitemfood)
                    ...
                    if (itemfood.book_food.length == 0) {
                      ...
                      // props.updateQuantity({ type: TYPE_UPDATE_CART.PROGRAM, status: CALCULATION.ADD, data: itemfood, category: CATEGORY.EAT_AT_SHOP, index, indexitemfood })
                  ...
                  //     props.updateQuantity({ type: TYPE_UPDATE_CART.PROGRAM, status: CALCULATION.SUBTRACTION, data: itemfood, category: CATEGORY.EAT_AT_SHOP, index, indexitemfood })
                  //     clearTimeout(timeOut.current)
                  //     timeOut.current = setTimeout(() => {
                  //       if (data.program_store[index].food[indexitemfood].count_book_food < 0) return
                  //       handleSubBookFood({
                  ...
                  //   }
                  //   return
                  //   props.updateQuantity({ type: TYPE_UPDATE_CART.CATEGORY, status: CALCULATION.SUBTRACTION, data: itemfood, category: CATEGORY.EAT_AT_SHOP })
                  //   clearTimeout(timeOut.current)
                  //   timeOut.current = setTimeout(() => {
